...
serve(async (req) => {
  ...
  try {
    ...
    addLog(isAutomated ? '🤖 Starting daily refresh pipeline (automated)' : '🔄 Starting daily refresh pipeline (manual)');
    ...
    await supabase
      ...
      .insert({
        ...
        action_details: { trigger_source: isAutomated ? 'automated' : 'manual' }
      });
