...
const CompanyArchive = () => {
  ...
  const decodedCompany = company ? decodeArchiveSlug(company) : "";
  const displayName = toTitleCase(decodedCompany);
  ...
  // Fetch all movies with production companies and filter client-side
  const { data, isLoading, isFetched, isPlaceholderData } = useQuery({
    queryKey: ["companyMovies", decodedCompany, offset],
    queryFn: async () => {
      // Fetch all movies that have production_companies
      const { data: allMovies, error } = await supabase
        .from("movies")
        .select("id, title, year, rating, imdb_rating, imdb_votes, genres, poster, local_poster_url, imdb_id, production_companies")
        .not("production_companies", "is", null)
      ...
      const filtered = (allMovies || []).filter((movie) => {
        if (!movie.production_companies || !Array.isArray(movie.production_companies)) return false;
        return movie.production_companies.some((comp: any) => {
          const compName = comp.name || comp;
          return compName.toLowerCase() === decodedCompany.toLowerCase();
        });
      });
  ...
  // SEO data
  const seoTitle = `${displayName} Movies - Films by ${displayName} | CineMatch`;
  const seoDescription = `Explore movies produced by ${displayName}. Browse ${totalCount} films with ${displayName} as production company. Complete movie list with ratings and details on CineMatch.`;
  const breadcrumbSchema = generateBreadcrumbSchema([
    { name: 'Home', url: '/' },
    { name: 'Movies', url: '/movies' },
    { name: displayName, url: `/company/${company}` },
  ]);
  ...
  if ((isLoading || !isFetched) && offset === 0) {
    return (
      <>
        ...
        <ArchiveLayout
          title={displayName}
          breadcrumbs={[{ label: displayName, href: `/company/${company}` }]}
          movieCount={0}
        >
